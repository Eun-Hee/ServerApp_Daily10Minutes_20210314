...
//                    500 - 서버 내부의 로직 에러
                    ...
                    if (code == 200) {
//                    정상 신청 완료 => 서버가 최신 상태값 다시 내려줌 => 다시 파싱해서 UI 반ㅇㅇ
...
//                            포기 API : 프로젝트 데이터 X. => 프로젝트 상태를 새로 조회해서 반영해야함.
                            ...
                            ServerUtil.getRequestProjectDetail(mContext, mProject.id, object : ServerUtil.JsonResponHandler {
                                override fun onResponse(json: JSONObject) {

                                    val dataObj = json.getJSONObject("data")
                                    val projectObj = json.getJSONObject("project")
