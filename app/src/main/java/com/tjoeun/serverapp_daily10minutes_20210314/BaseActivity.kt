...
//    멤버변수로 UI요서를 만들어둬야 물려받아서 사용가능
//    만든는건 멤버변수 => 대입은 커스텀액션바완성 이후에 가능 (나중에 가능)
...
//            기본 액션바가 실제로 존재할때 (let 의 역할)만, 커스터마이징 진행
            setCustomActionBar()
...
//    이 클래스에 적인 변수/함수는 다른 모든 액티비티가 물려받게 된다
//    모든 화면에 공통적인 기능을 만들때는 여기서 처리
...
//        좌우 여백을 제거해야 함
//        여백을 없애려면? => 액션바-툴바 역백 제거 (내부 공간 0으로 설정)
